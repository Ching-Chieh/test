...
class Filtered_MACD_strategy(Strategy):
    window_fast = 12
    window_slow = 26
    window_signal = 9
    stoploss_pct_long = 2
    ...
    def init(self):
        ...
        if self.macd_ma_type == "EMA":
            ...
            self.macd, self.signal, _ = self.I(MACD_SMA,
                                               close,
                                               window_fast=self.window_fast,
                                               window_slow=self.window_slow,
    ...
    def next(self):
        ...
        if (not self.position) and (self.daily_high is not None):
            # 多單進場
            filter_1 = close > self.daily_high and high[-1] >= high[-4:-1].max()
            if crossover(self.macd, self.signal) and filter_1:
            ...
            # 空單進場
            filter_2 = close < self.daily_low and low[-1] <= low[-4:-1].min()
            if crossover(self.signal, self.macd) and filter_2:
...
def generate_df_trades(trades):
    trades = trades[['Size', 'EntryBar', 'ExitBar', 'EntryPrice', 'ExitPrice',
                   'Entry_macd', 'Entry_signal', 'PnL']].rename(columns={
                   "Size": "type",
                   "Entry_macd": "macd",
                   "Entry_signal": "signal",
                   "PnL": 'Profit or loss'
    ...
    trades = trades[['type', 'date', 'EntryPrice', 'ExitPrice', 'macd', 'signal', 'Profit or loss', 'EntryBar', 'ExitBar']]
    trades[['macd', 'signal']] = trades[['macd', 'signal']].round(1)
    return trades
#%% bt
df = pd.read_csv("5min_data_FITX_1.TF.log", sep = " ", usecols=range(6))
df = prepare_df(df) # 回測起始日期: 20210601
df = df[df.index >= '2021-04-01']

cash = 1_000_000_000
bt = Backtest(df, Filtered_MACD_strategy,
              cash=cash,
              trade_on_close=True,
...
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
#%% summary
stats = bt.run(window_fast = 12, window_slow = 26, window_signal = 9,
               stoploss_pct_long = 2, stoploss_pct_short = 2,
               daily_high_low_time = 925, macd_ma_type = "EMA")
df_trades = generate_df_trades(stats._trades)
# 毛利
