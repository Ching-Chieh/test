...
class Middle_line_strategy(Strategy):
    stoploss_pct_long = 2
    ...
    def next(self):
        ...
        if self.data.time[-1] == self.daily_high_low_time:
            ...
            self.middle_line = self.daily_low + (self.daily_high - self.daily_low) * self.m
        
        close = self.data.Close[-1]
        
        # 進場
        if self.middle_line is not None:
            long_entry_condition = close > self.middle_line
            short_entry_condition = close < self.middle_line
