    ...
    def next(self):
        ...
        if self.daily_high is not None:
            if not self.position:
                value = self.data.dailyrange_lastday[-1]*self.mult
                long_entry_condition = close > self.daily_high + value
                short_entry_condition = close < self.daily_low - value
                # 多單
