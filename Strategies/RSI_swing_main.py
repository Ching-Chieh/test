...
import pandas as pd
from RSI_swing import RSI_swing_strategy
#%% functions
def prepare_df(df):
...
cash = 1_000_000_000
bt = Backtest(df, RSI_swing_strategy,
              cash=cash,
              trade_on_close=True,
...
param_grid = {
    ...
    "rsi_short_len": range(4, 40+1, 4),
    "rsi_long_len": range(8, 60+1, 4),
...
print('\nBest parameters:\n', best_params)
#%% best parameters
stats = bt.run(stoploss_pct = 3, rsi_short_len = 19, rsi_long_len = 50, daily_high_low_time = 935)
print('\nNumber of trades=', len(stats['_trades']))
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
