...
# bt._broker.keywords['cash']可以取得cash
#%% sambo
...
# max_drawdown, max_drawdown_pct, longest_drawdown_period

def calc_max_drawdown(stats_equity_curve: pd.DataFrame):
    df = (
            stats_equity_curve
            .copy()
            [['Equity']]
            .rename(columns={'Equity': 'equity'})
            .assign(
                cum_max = lambda x: x['equity'].cummax(),
                drawdown = lambda x: x['cum_max'] - x['equity'],
            )
        )
    
    max_drawdown = df['drawdown'].max()
    temp = df[df['drawdown'] == 0]
    periods = temp.index[1:].to_pydatetime() - temp.index[:-1].to_pydatetime()
    longest_drawdown_period = max(periods)
    
    return dict(max_drawdown = max_drawdown, longest_drawdown_period = longest_drawdown_period)

tmp = calc_max_drawdown(stats['_equity_curve'])
max_drawdown = tmp['max_drawdown']
longest_drawdown_period = tmp['longest_drawdown_period'].days # stats['_equity_curve']['DrawdownDuration'].max()
...
print(f"Longest drawdown period= {longest_drawdown_period:.0f} days")
