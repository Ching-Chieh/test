...
class Ichimoku4_Strategy(Strategy):
    s = 9
    m = 26
    l = 52
    exit_high_n = 2
    ...
    def init(self):
        ...
        self.base_line       = self.I(func, high, low, self.m)
        self.leading_A       = 0.5*(self.conversion_line + self.base_line)
        self.leading_B       = self.I(func, high, low, self.l)
    ...
    def next(self):
        ...
        if not self.position:
            long_entry_condition = conversion_line[-2] < base_line[-2] and conversion_line[-1] >= base_line[-1]
            val = min(self.leading_A[-self.m], self.leading_B[-self.m])
            short_entry_condition = close > val and close < val
            ...
            long_exit_condition = conversion_line[-2] > base_line[-2] and conversion_line[-1] <= base_line[-1]
