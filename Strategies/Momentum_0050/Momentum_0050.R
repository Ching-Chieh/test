...
# library(writexl)
# write_xlsx(df, "prices_R.xlsx")
df <- read_excel("prices_R.xlsx") %>% 
  mutate(
    date = ymd(date),
...
tmp <- past_Nm_return_df %>% 
  ...
  mutate(
    name = str_replace(name, "_pastNm", ""),
    g = ntile(value, 5)
  )
long_stocks  <- tmp %>% filter(g == 5) %>% pull(name)
short_stocks <- tmp %>% filter(g == 1) %>% pull(name)
...
past_Nm_return_df %>% 
  slice(1) %>% 
  select(-year, -month, -ends_with("_pastNm")) %>% 
  unlist %>% 
  mean %>% 
  round(4)# Buy-and-Hold
# for loop ----------------------------------------------------------------
