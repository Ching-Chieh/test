...
import pandas as pd
from DMI30min import DMI30minStrategy
#%% functions
def prepare_df(df):
...
#%% bt
df = pd.read_csv("30min_data_FITX_1.TF.log", sep=" ", usecols=range(6)) #  20190102 845 ~ 20251031 1315
df = prepare_df(df)
df = df[df.index > '2021-01-01'] # 回測起點 日期: 20210601

cash = 1_000_000_000
bt = Backtest(df, DMI30minStrategy,
              cash=cash,
              trade_on_close=True,
...
import numpy as np
param_grid = {
    "adx_value": range(2, 40, 2),
    "stoploss_pct": np.arange(0.2, 5+0.01, 0.2)
}
...
#%% best_params
stats = bt.run(adx_value = 32, stoploss_pct = 4.2)
