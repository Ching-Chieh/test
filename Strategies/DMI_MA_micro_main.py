...
import pandas as pd
from DMI_MA_micro import DMI_MA_Strategy
#%% functions
def prepare_df(df):
...
#%% bt
df = pd.read_csv("5min_data_FITMN_1.TF.log", sep=" ", usecols=range(6))# 有夜盤 20250331 935 ~ 20251121 1340
df = prepare_df(df)
# df = df[df.index > '2025-09-01'] # 回測起點 日期: 20250403

cash = 1_000_000_000
bt = Backtest(df, DMI_MA_Strategy,
              cash=cash,
              trade_on_close=True,
...
param_grid = {
    "adx_value": range(15, 40+1, 5),
    "ma_short": range(5, 40+1, 5),
    "ma_long": range(5, 65+1, 5),
}
...
stats = bt.run(adx_value = best_params['adx_value'], # 15
               ma_short = best_params['ma_short'],   # 5
               ma_long = best_params['ma_long'])     # 25
print(f"\nNet profit= {stats['Equity Final [$]']-cash:,.0f}")
