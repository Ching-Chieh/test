...
def generate_df_trades(trades):
    ...
    trades[['+DI', '-DI', 'ADX']] = trades[['+DI', '-DI', 'ADX']].round(1)
    return trades
#%% bt
df = pd.read_csv("long_5min_data_FITX_1.TF.log", sep=" ", usecols=range(6))
df = prepare_df(df)
...
#%% try
stats = bt.run(adx_value = 20, stoploss_pct = 1)
print(stats)
...
param_grid = {
    "adx_value": range(15, 21+1, 2), # range(10, 41, 2),
    "stoploss_pct": frange(1, 3.1, 0.5) # np.arange(1, 3.1, 0.5).tolist()
}
