...
cash = 1_000_000_000
bt = Backtest(df, Momentum_Strategy,
              cash=cash,
              trade_on_close=True,
...
param_grid = {
    "period": [5] + list(range(10, 120+1, 10)),
    "sma_period": [5] + list(range(10, 120+1, 10)),
    "baseline": range(-100, 100+1, 10)
}
...
print('\nBest parameters:\n', best_params)
#%% best parameters
stats = bt.run(period = 75, sma_period = 95, baseline = 20)
print('\nNumber of trades=', len(stats['_trades']))
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
