...
import pandas as pd
from OpenBO import OpenBO_strategy
#%% functions
def prepare_df(df):
    daily = df.groupby("date").first().reset_index(drop=False, inplace=False)[['date', 'open']].rename(columns={'open':'daily_open'})
    ...
    df = df.merge(daily, how='left', on='date')
    ...
    return df[["date", "time", "Open", "High", "Low", "Close", "daily_open"]]
#%% bt
...
cash = 1_000_000_000
bt = Backtest(df, OpenBO_strategy,
              cash=cash,
              trade_on_close=True,
...
param_grid = {
    ...
    "long_mult": np.arange(0.1, 2.9+0.001, 0.4).round(1).tolist(),
    "short_mult": np.arange(0.1, 2.9+0.001, 0.4).round(1).tolist()
    # "_short": [True, False]
}
...
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
#%% short
print(OpenBO_strategy._short)
#%% best parameters
stats = bt.run(stoploss_pct_long = 1.8, stoploss_pct_short = 0.64, long_mult = 0.68, short_mult = 0.15)
print("\nNumber of trades=", len(stats['_trades']))
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
