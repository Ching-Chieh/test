...
import pandas as pd
from HL_passage_trailing_stoploss import HL_passage_trailing_stoploss_strategy
#%% functions
def prepare_df(df):
...
cash = 1_000_000_000
bt = Backtest(df, HL_passage_trailing_stoploss_strategy,
              cash=cash,
              trade_on_close=True,
...
import numpy as np
param_grid = {
    "high_period": range(3, 19+1, 2),
    "low_period": range(3, 19+1, 2),
...
#%% best parameters
stats = bt.run(high_period = 5, low_period = 3, stoploss_pct_long = 2.5, stoploss_pct_short = 3)
print("\nNumber of trades=", len(stats['_trades']))
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
