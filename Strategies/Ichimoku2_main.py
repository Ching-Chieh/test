...
import pandas as pd
from Ichimoku2 import Ichimoku2_Strategy
#%% functions
def prepare_df(df):
...
#%% bt
df = pd.read_csv("20min_data_FITXN_1.TF.log", sep=" ", usecols=range(6)) # 有夜盤 20181228 1500 ~ 20251031 1325
df = prepare_df(df)
df = df[df.index > '2021-05-01'] # 回測起點 日期: 20210601
...
stats = bt.run(s = 6, l = 32, exit_high_n = 5)
print("\nNumber of trades:", len(stats['_trades']))
print(f"\nNet profit= {stats['Equity Final [$]']-cash:,.0f}") # 3,353,200
