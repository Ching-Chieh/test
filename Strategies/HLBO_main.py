...
import pandas as pd
from HLBO import HLBO_strategy
#%% functions
def prepare_df(df):
...
#%% short
print(HLBO_strategy._short)
#%% best parameters
stats = bt.run(stoploss_pct_long = 1.8, stoploss_pct_short = 2.9, long_mult = 0.1, short_mult = 1.6, daily_high_low_time = 940)
print("\nNumber of trades=", len(stats['_trades']))
print(f"Net profit= {stats['Equity Final [$]']-cash:,.0f}")
